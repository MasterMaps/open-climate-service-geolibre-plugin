...
export function stepLabel(dim: DimState, i: number): string {
  const v = dim.steps[i];
  if (v == null) return String(i);
  // Steps come from two places. A regular axis is generated by `iterateTemporal`, which
  // already trims to the granularity of its step. An *irregular* axis (a dekadal store has
  // no ISO step, so STAC lists explicit `values`) arrives as raw ISO timestamps, which
  // would read "2024-12-21T00:00:00Z" on every position. Trim those to match, while
  // leaving ordinal axes — day-of-year numbers, category names — untouched.
  if (typeof v === "string") {
...
/** Label an irregular temporal axis from its own timestamps, since it has no ISO step.
 *
 * A cadence with unequal periods cannot be an ISO duration, so the datacube extension gives
 * it `step: null` and lists explicit `values` — and OCS publishes no cadence name alongside
 * them. Dekads are recognisable from the timestamps themselves: three per month, always on
 * the 1st, 11th and 21st, with the third running 8–11 days. Anything else stays "Irregular"
 * rather than being guessed at.
 */
function irregularResolutionLabel(values: unknown): string {
  if (!Array.isArray(values) || values.length < 2) return "Irregular";
  const days = values
    .map((v) => new Date(String(v)).getUTCDate())
    .filter((d) => !Number.isNaN(d));
  if (days.length !== values.length) return "Irregular";
  const onDekadStarts = days.every((d) => d === 1 || d === 11 || d === 21);
  // Require a non-first dekad, so a monthly axis (every value on the 1st) is never mislabelled.
  const hasLaterDekad = days.some((d) => d === 11 || d === 21);
  return onDekadStarts && hasLaterDekad ? "10-daily (dekad)" : "Irregular";
}
...
function temporalResolutionLabel(dimensions: StacJson): string | null {
  for (const v of Object.values(dimensions)) {
    const dim = v as StacJson;
    if (dim.type !== "temporal") continue;
    if (typeof dim.step === "string" && dim.step) {
      return _ISO_STEP_LABELS[dim.step] ?? dim.step;
    }
    // `step: null` is a declaration, not an omission — an irregular cadence. Reporting no
    // resolution at all left the panel silent about a dataset that does have one.
    if (dim.step === null) return irregularResolutionLabel(dim.values);
  }
  return null;
}
